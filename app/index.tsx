...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const loadTopics = async () => {
      ...
      try {
        ...
        if (isActive) setFeaturedTopics(topics);
      } catch {
      } finally {
  ...
  useEffect(() => {
    ...
    const runSearch = async () => {
      ...
      try {
        ...
        if (isActive) setResults(response);
      } catch {
        if (isActive) setError('Unable to fetch knowledge right now. Please try again.');
      } finally {
        if (isActive) setIsSearching(false);
      }
    };

    if (debouncedQuery.trim().length >= 2) runSearch();
    else {
      setResults([]);
      setError(null);
  ...
  const filteredTopics = useMemo(() => {
    if (selectedCategory === 'Featured') return featuredTopics;
          ...
          <>
            ...
            <View style={styles.section}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoriesRow}>
                <CategoryChip label="Featured" isActive={selectedCategory === 'Featured'} onPress={() => setSelectedCategory('Featured')} />
                {CATEGORIES.map((category) => (
                  <CategoryChip key={category} label={category} isActive={selectedCategory === category} onPress={() => setSelectedCategory(category)} />
                ))}
              </ScrollView>
            </View>
              ...
              {isLoadingTopics ? (
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.featuredRow}>
                  {filteredTopics.map((topic) => (
                    <FeaturedTopicCard key={topic.title} topic={topic} onPress={() => handleOpenTopic(topic.title)} />
                  ))}
                </ScrollView>
              ) : (
                <Text style={[styles.emptyState, { color: isDark ? '#94a3b8' : '#64748b' }]}>
...
const styles = StyleSheet.create({
  ...
  feedbackRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  feedbackText: { fontSize: 14, marginLeft: 12 },
  errorBox: {
    borderWidth: 1,
  ...
  categoriesRow: { paddingRight: 20 },
  featuredRow: { paddingRight: 20 },
  loadingContainer: { flexDirection: 'row', alignItems: 'center', paddingVertical: 24 },
  loadingText: { fontSize: 14, marginLeft: 12 },
});
